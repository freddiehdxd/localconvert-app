...
pub const TOOLS: &[(&str, &str, &str)] = &[
    ...
    ("gswin64c", "Ghostscript", "PDF operations"),
...
pub fn get_tool_path(tool_name: &str) -> String {
    ...
    // Fall back to command name (will work if tool is in PATH)
    tool_name.to_string()
}
...
fn get_common_paths(tool_name: &str) -> Vec<String> {
    ...
    match tool_name {
        ...
        "pandoc" => {
            ...
            // Check for versioned Pandoc folders in standard locations and Downloads
            // Supports: "Pandoc-3.1.12", "pandoc-3.6.4", nested extracts like "pandoc-3.1.12-windows-x86_64\pandoc-3.1.12"
                ...
                if let Ok(entries) = std::fs::read_dir(base) {
                    for entry in entries.flatten() {
                        ...
                        if name_lower.starts_with("pandoc") {
                            ...
                            // Direct: pandoc.exe in folder
                            paths.push(format!("{}\\pandoc.exe", folder_path));
                            // Nested: check for subfolder with pandoc.exe (e.g., pandoc-3.1.12-windows-x86_64\pandoc-3.1.12\pandoc.exe)
        ...
        "gswin64c" => vec![
            format!("{}\\gs\\gs10.02.1\\bin\\gswin64c.exe", program_files),
            format!("{}\\gs\\gs10.00.0\\bin\\gswin64c.exe", program_files),
        ],
        "tesseract" => vec![
        ...
        "pymupdf" => vec![], // Python module, no path needed
        _ => vec![],
    }
}
...
fn get_tool_version_at_path(path: Option<&str>, tool_name: &str) -> Option<String> {
    let cmd = path.unwrap_or(tool_name);
    ...
    if tool_name == "soffice" || tool_name == "gswin64c" {
        if let Some(p) = path {
            if std::path::Path::new(p).exists() {
...
pub fn get_tool_download_url(tool_name: &str) -> Option<&'static str> {
    match tool_name {
        ...
        "gswin64c" => Some("https://github.com/ArtifexSoftware/ghostpdl-downloads/releases/download/gs10021/gs10021w64.exe"),
...
pub fn get_gpu_encoder_for_format(format: &str, gpu_info: &GpuInfo, preferred: Option<&str>) -> Option<String> {
    ...
    if let Some(pref) = preferred {
        if let Some(encoder) = gpu_info.encoders.iter().find(|e| e.name == pref) {
            ...
            // For WebM, the preferred h264 encoder is incompatible, so continue to find AV1 encoder
        }
    }
